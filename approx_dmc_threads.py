...
def get_circles(original_image, filtered_image, scale):
    ...
    for small_circle in np.round(small_circles[0, :]).astype("int"):
        ...
        scaled_up_circles.append(
            (int(x / scale), int(y / scale), int((r / scale) * shrink_amount))
        )
...
def get_largest_circle(file_path, debug=False):
    ...
    if debug:
        ...
        dim = (
            int(filtered_image.shape[1] / scale),
            int(filtered_image.shape[0] / scale),
        )
        save_debug_circles(
            cv2.resize(debug_filtered_image, dim), circles, largest_circle
        )
...
def generate_palette(circle_image, original_image, output_file=False):
    ...
    with_percentage = [
        (i, color_counter[i] / len(circle_area) * 100.0)
        for i, _ in color_counter.most_common()
    ]
    limit_low_occuring_threads = 2  # %
    filtered = [
        color for color in with_percentage if color[1] > limit_low_occuring_threads
    ]
    with_dmc = [
